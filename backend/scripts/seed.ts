/**
 * Demo seed script — заливает 5 тестовых пользователей для демонстрации.
 * Запуск: npx ts-node --project tsconfig.json scripts/seed.ts
 * Требует: postgres запущен на localhost:5433 (docker-compose up)
 */
...
async function main() {
  ...
  const md = [
    ...
    '|-----|---------|-------|---------------|',
    ...users.map((u) =>
      `| ${u.firstName} ${u.lastName} | @${u.nickname} | \`${u.email}\` | ${u.activityField} |`,
    ),
    '',
    '## Как войти',
    '1. Открой приложение',
